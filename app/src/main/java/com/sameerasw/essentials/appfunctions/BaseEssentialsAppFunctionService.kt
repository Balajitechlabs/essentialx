...
abstract class BaseEssentialsAppFunctionService : AppFunctionService() {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun setSoundMode(mode: String): AppFunctionResult =
        withContext(Dispatchers.IO) {
            val soundModeType =
                when (mode.uppercase()) {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun setWallpaperDim(dimLevel: Int): AppFunctionResult =
        withContext(Dispatchers.IO) {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun setAlwaysOnDisplay(mode: String): AppFunctionResult =
        withContext(Dispatchers.IO) {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun turnScreenOff(): AppFunctionResult =
        withContext(Dispatchers.IO) {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun setLowPowerMode(enabled: Boolean): AppFunctionResult =
        withContext(Dispatchers.IO) {
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun listFreezeTags(): List<FreezeTagSummary> =
        withContext(Dispatchers.IO) {
            val repo = SettingsRepository(applicationContext)
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun freezeTagApps(tagId: String): AppFunctionResult =
        withContext(Dispatchers.IO) {
            val repo = SettingsRepository(applicationContext)
            val tags = repo.getFreezeTags()
            val targetTag =
                tags.find {
                    it.id.equals(tagId, ignoreCase = true) ||
                        it.name.equals(
                            tagId,
                            ignoreCase = true,
                        )
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun unfreezeTagApps(tagId: String): AppFunctionResult =
        withContext(Dispatchers.IO) {
            val repo = SettingsRepository(applicationContext)
            val tags = repo.getFreezeTags()
            val targetTag =
     ...
     */
    @AppFunction(isDescribedByKDoc = true)
    suspend fun listAutomations(): List<AutomationSummary> =
        withContext(Dispatchers.IO) {
    ...
    suspend fun toggleAutomation(
        ...
        withContext(Dispatchers.IO) {
            val automations = DIYRepository.automations.value
            val target =
                automations.find { it.id.equals(automationId, ignoreCase = true) }
    ...
    suspend fun createAutomation(params: CreateAutomationParams): AppFunctionResult =
        withContext(Dispatchers.IO) {
            ...
            val trigger: Trigger? =
                if (autoType == Automation.Type.TRIGGER) {
            ...
            val state: State? =
                if (autoType == Automation.Type.STATE) {
