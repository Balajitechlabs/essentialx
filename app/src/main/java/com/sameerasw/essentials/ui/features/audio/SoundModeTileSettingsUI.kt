 ...
 * File: SoundModeTileSettingsUI.kt
...
fun SoundModeTileSettingsUI(
    ...
    var disabledModes by remember {
        mutableStateOf(
            if (disabledString.isEmpty()) {
                emptyList()
            } else {
                disabledString.split(
                    ",",
                )
            },
        )
            ...
            val fromMode: String =
                when {
                    from.index < originalActiveSize ->
                        activeModes.getOrNull(from.index)
                            ?: return@rememberReorderableLazyListState

                    from.index == originalActiveSize -> "separator"
                    else ->
                        disabledModes.getOrNull(from.index - originalActiveSize - 1)
                            ?: return@rememberReorderableLazyListState
                }
